  ...
  "relative w-full border-2 p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground",
  {
    variants: {
      variant: {
        ...
        success:
          "border-success bg-success/10 text-success [&>svg]:text-success",
        warning:
          "border-warning bg-warning/10 text-warning [&>svg]:text-warning",
...
>(({ className, ...props }, ref) => (
  <h5
    ref={ref}
    className={cn(
      "mb-1 font-serif text-lg font-bold leading-none tracking-tight",
      className
    )}
    {...props}
  />
